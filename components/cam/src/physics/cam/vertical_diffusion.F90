...
!use const_init ,      only : latvals 
use phys_grid  ,      only : get_lat_all_p,get_rlat_all_p
!use physics_ma,       only : hello_world 
implicit none
private
save
...
subroutine vd_register()
  ...
  call pbuf_add_field('pbl_h',     'global', dtype_r8, (/pcols/),      pbl_h_idx)
  call pbuf_add_field('pbl_h_dp',  'global', dtype_r8, (/pcols/),     pbl_h_dp_idx)
  call pbuf_add_field('f_canshu',     'global', dtype_r8, (/pcols/),  f_canshu_idx)
  call pbuf_add_field('f_star',     'global', dtype_r8, (/pcols/),    f_star_idx)
  call pbuf_add_field('tau_star',     'global', dtype_r8, (/pcols/),  tau_star_idx)
...
subroutine vertical_diffusion_init(pbuf2d)
  ...
  integer        :: k ,j          ! Vertical loop index
...
subroutine vertical_diffusion_tend( &
  ...
  use pbl_iap,            only : calc_pbl_h_vector ,calc_pbl_h_vector_no_iterate,calc_pbl_h_deeplearn,&
        yunliang, calc_pblh_tbf
...
!=======================================ma ============== 
  ...
  real(r8) :: n2(pcols,pver)                                      ! richardson number (HB output) mayubin
  ...
  real(r8), pointer :: pblh(:)!,f_star(:),tau_star(:),t_surface(:),&
                       !n_pinglv(:) !, pbl_h(:)
...
!===============================ma 22
  real(r8) :: pbl_h(pcols)
  real(r8) :: pbl_h_dp(pcols)
  real(r8) :: f_canshu(pcols),f_star(pcols),tau_star(pcols),t_surface(pcols),&
              n_pinglv(pcols,pver) 
...
!===============================ma 22
  real(r8)  :: latvals(pcols)
  real(r8)  :: pressure_pver(pcols,pver)
...
!  call pbuf_get_field(pbuf, pbl_h_idx,     pbl_h)
...
!calculation  of cloud fractional amount    ! 2022-9-20 
  ...
  call yunliang(ncol,obklen(:ncol),ftem_prePBL(:ncol,31),n_l(:ncol))   
    ...
    f_canshu  = 2_r8 *7.292_r8*0.00001_r8*sin(latvals)
    f_star    = cam_in%shf(:ncol)
    tau_star  = ustar(:ncol)
    t_surface = thvs(:ncol)
!jisuan N2 
!mayubin 2022-10-10
!youyuzi de chayi henda zaocheng jisusan bulianxu ,dui zi pingjun 

        do k = 1,pver
           zi_mean(k) = 0_r8
           zi_sum = 0_r8
           do i = 1,ncol 
           zi_sum = zi_sum + state%zi(i,k)
           enddo
           zi_mean(k) = zi_sum/ncol
        enddo

!     do i = 1,ncol
!        do k = 1,pver-1
!                n_pinglv(i,k) =(gravit/th(i,k))*(th(i,k) - th(i,pver))/( state%zi(i,k) - state%zi(i,pver) )
!                
!        end do 
!        n_pinglv(i,pver) = n_pinglv(i,pver - 1)
!     end do 
!    n_pinglv  = ri(:ncol,:pver)

     n_pinglv(:ncol,1) = 0._r8

     do k = 2, pver
        do i = 1, ncol

                n_pinglv(i,k) =(gravit/th(i,k))*(th(i,k-1) - th(i,k))/(                state%zm(i,k-1) - state%zm(i,k) )
                
        end do 
     end do 

     n_pinglv(:ncol,pverp) = n_pinglv(:ncol,pver)
!    zm_shuchu  = state%zm
...
!        do i = 1,pver 
!!                print*,'0920print','zhe shi di ',i,' ceng','qi ya shi ',state%pmid(1,i),&
!                'rh = ',ftem_prePBL(1,i),'zi = ',state%zi(1,i)
!        enddo
...
!==================================yy=========ma 2022============================ 
  ...
  call outfld('pbl_h', pbl_h(:), pcols , lchnk  ) 
  call outfld('pbl_h_dp',pbl_h_dp(:),pcols,lchnk)
  call outfld('n_l', n_l(:),pcols, lchnk)
  
  call outfld('f_canshu', f_canshu(:),pcols, lchnk)
  call outfld('f_star', f_star(:),pcols, lchnk)
  ...
  if (.not. do_pbl_diags) then
     ...
     call qsat(state%t(:ncol,:), state%pmid(:ncol,:), &
          tem2(:ncol,:), ftem(:ncol,:))
     ftem_prePBL(:ncol,:) = state%q(:ncol,:,1)/ftem(:ncol,:)*100._r8
!zhe ge  shi jisuan xiangduishidu de gongshi ,keneng hui dui wo de jisuan
!youyong mayubin 2022-9-20
...
!        pblh =PBL_H!mayubin
  ...
  if (.not. do_pbl_diags) then
     ...
     call outfld( 'PBLH'         , pblh,                      pcols, lchnk )!mayubin
