...
!==============================================================================
  ...
  subroutine diag_init_dry(pbuf2d)
    ...
    call addfld ('pbl_h',      horiz_only , 'A', 'm'  , 'Boundary layer height'   ) !mayubin 22-7-7 
    call addfld ('pbl_h_dp',   horiz_only , 'A', 'm'  , 'Boundary layer height from deep learning' )!mayubin22-7-8
    call addfld ( 'n_l',       horiz_only ,  'A','%'  ,  'cloud fractional amount ' )!mayuybin22-9-20

    call addfld ( 'f_canshu',       horiz_only ,  'A','s^-1'  ,  'ke shi can shu ' )!mayuybin22-9-26
    call addfld ( 'f_star',       horiz_only ,  'A','wm^-2'  ,  'gan re tong liang ' )!mayuybin22-9-26
  ...
  subroutine diag_init_moist(pbuf2d)
    ...
    pbl_h_idx  = pbuf_get_index('pbl_h',errcode=ierr)!mayubin
    pbl_h_dp_idx = pbuf_get_index('pbl_h_dp',errcode = ierr)!mayubin
    n_l_idx     = pbuf_get_index( 'n_l',errcode = ierr) !mayubin2022-9-20

    f_canshu_idx     = pbuf_get_index( 'f_canshu',errcode = ierr) !mayubin2022-9-20
    f_star_idx     = pbuf_get_index( 'f_star',errcode = ierr) !mayubin2022-9-20
  ...
  subroutine diag_physvar_ic (lchnk,  pbuf, cam_out, cam_in)
    ...
    if( write_inithist() .and. moist_physics ) then
      ...
      if (pblh_idx > 0) then
        ...
        call outfld('pbl_h',pbl_h,pcols,lchnk) !mayubin 22-7-7

        !==============
        call pbuf_get_field(pbuf,pbl_h_dp_idx,pbl_h_dp)
        call outfld('pbl_h_dp',pbl_h_dp,pcols,lchnk) !mayubin 22-7-7

        call pbuf_get_field(pbuf,f_canshu_idx,f_canshu)
        call outfld('f_canshu',f_canshu,pcols,lchnk) !mayubin 22-9-26

        call pbuf_get_field(pbuf,f_star_idx,f_star)
        call outfld('f_star',f_star,pcols,lchnk) !mayubin 22-9-26

        call pbuf_get_field(pbuf,tau_star_idx,tau_star)
        call outfld('tau_star',tau_star,pcols,lchnk) !mayubin 22-9-26

        call pbuf_get_field(pbuf,t_surface_idx,t_surface)
        ...
        call pbuf_get_field(pbuf,n_l_idx,n_l)
        call outfld('n_l',n_l,pcols,lchnk) !mayubin 22-9-20
